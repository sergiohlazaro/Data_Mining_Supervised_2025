...
# print("\n")
# -------------------------------
# SELECCIÓN DE CARACTERÍSTICAS MULTIVARIANTE (FSS)
# -------------------------------
from sklearn.ensemble import RandomForestClassifier
...
for target in target_vars:
    print(f"FSS para target: {target}")

    X = df_clean.drop(columns=target_vars)
    y = df_clean[target]

    # -------------------
    # MÉTODO DE FILTRADO: Random Forest Importancia
    # -------------------
    rf = RandomForestClassifier(n_estimators=100, random_state=42)
    rf.fit(X, y)
    importancias = pd.Series(rf.feature_importances_, index=X.columns).sort_values(ascending=False)
    importancias.to_csv(f"resultados/fss/{target}_fss_filtrado_importancia.csv")

    plt.figure(figsize=(10, 6))
    sns.barplot(x=importancias[:10].values, y=importancias[:10].index, hue=importancias[:10].index, palette="coolwarm", legend=False)
    plt.title(f"Top 10 Variables - FSS por Importancia (Random Forest) - {target}")
    plt.xlabel("Importancia")
    plt.tight_layout()
    plt.savefig(f"imgs1/fss/{target}_fss_filtrado_rf_importancia.png")
    plt.close()

    # Cross-val predict y métricas
    y_pred_rf = cross_val_predict(rf, X, y, cv=StratifiedKFold(5, shuffle=True, random_state=42))
    resumen_fss.append({
        'Target': target,
        'Tipo': 'FSS (Filtrado)',
        'Modelo': 'RandomForest Importancia',
        'Accuracy': round(accuracy_score(y, y_pred_rf), 4),
        'F1': round(f1_score(y, y_pred_rf, zero_division=0), 4),
        'Precision': round(precision_score(y, y_pred_rf, zero_division=0), 4),
        'Recall': round(recall_score(y, y_pred_rf, zero_division=0), 4)
    })

    # -------------------
    # MÉTODO WRAPPER: RFE con Regresión Logística
    # -------------------
    selector = RFE(LogisticRegression(max_iter=1000, solver='liblinear', random_state=42), n_features_to_select=10)
    selector = selector.fit(X, y)
    seleccionadas = X.columns[selector.support_].tolist()

    df_rfe = pd.DataFrame({
        "Seleccionada": selector.support_,
        "Ranking": selector.ranking_
    }, index=X.columns).sort_values(by="Ranking")
    df_rfe.to_csv(f"resultados/fss/{target}_fss_wrapper_rfe.csv")

    plt.figure(figsize=(10, 6))
    sns.barplot(x=df_rfe[df_rfe['Seleccionada']].loc[seleccionadas, "Ranking"],
                y=seleccionadas, hue=seleccionadas, palette="magma", legend=False)
    plt.title(f"Top Variables - FSS por Wrapper (RFE) - {target}")
    plt.xlabel("Ranking (menor es mejor)")
    plt.tight_layout()
    plt.savefig(f"imgs1/fss/{target}_fss_wrapper_rfe.png")
    plt.close()

    # Entrenar modelo con variables seleccionadas y evaluar con cross_val_predict
    X_sel = X[seleccionadas]
    model_logreg = LogisticRegression(max_iter=1000, solver='liblinear', random_state=42)
    y_pred_rfe = cross_val_predict(model_logreg, X_sel, y, cv=StratifiedKFold(5, shuffle=True, random_state=42))

    resumen_fss.append({
        'Target': target,
        'Tipo': 'FSS (Wrapper)',
        'Modelo': 'RFE (LogReg)',
        'Accuracy': round(accuracy_score(y, y_pred_rfe), 4),
        'F1': round(f1_score(y, y_pred_rfe, zero_division=0), 4),
        'Precision': round(precision_score(y, y_pred_rfe, zero_division=0), 4),
        'Recall': round(recall_score(y, y_pred_rfe, zero_division=0), 4)
    })

df_fss = pd.DataFrame(resumen_fss)

# -------------------------------
# ACTUALIZACIÓN DE LA TABLA CONSOLIDADA DE RESULTADOS
# -------------------------------
df_consolidado = pd.concat([
    df_supervisados.assign(Tipo="Modelos Supervisados"),
    df_j48_smote.assign(Tipo="J48 + SMOTE"),
...
Path("resultados/resumen").mkdir(parents=True, exist_ok=True)
df_consolidado.to_csv("resultados/resumen/resumen_modelos.csv", index=False)
print("\n📊 Tabla resumen de todos los modelos actualizada:")
print(df_consolidado.to_string(index=False))
print("\n")